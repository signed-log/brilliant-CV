...
test: test-image
    ...
    @docker run --rm --platform={{DOCKER_PLATFORM}} -v "$(pwd):/workspace" {{DOCKER_IMAGE}} bash -c "tt run --no-fail-fast && bash tests/panics/run.sh && bash tests/query/run.sh && bash tests/text/run.sh && bash tests/pdfua/run.sh"
